...
{
    if (ext) {
        // Para IDs estendidos (29 bits), o MCP2515 divide o ID da seguinte forma:
        buffer[MCP_EID0] = (uint8_t) (id & 0xFF);
        buffer[MCP_EID8] = (uint8_t) ((id >> 8) & 0xFF);
        
        uint8_t slot_sidl = (uint8_t) ((id >> 16) & 0x03);
        slot_sidl |= (uint8_t) (((id >> 16) & 0x1C) << 3);
        slot_sidl |= TXB_EXIDE_MASK; // Ativa a flag de ID Estendido no chip
        buffer[MCP_SIDL] = slot_sidl;
        
        buffer[MCP_SIDH] = (uint8_t) (id >> 21);
    } else {
        // Para IDs normais (11 bits)
        buffer[MCP_SIDH] = (uint8_t) (id >> 3);
        buffer[MCP_SIDL] = (uint8_t) ((id & 0x07) << 5);
        buffer[MCP_EID0] = 0;
        buffer[MCP_EID8] = 0;
    }
}
...
ERROR_t MCP2515_sendMessage(const TXBn_t txbn, const CAN_FRAME frame)
{
    ...
    // Adicionado o atributo de alinhamento para garantir que o DMA do ESP32 
    // leia corretamente o array alocado na Stack da Task
    uint8_t data[13] __attribute__((aligned(4))); 
    ...
    // Agora passará o ID completo sem decepar os bits superiores
    ...
    // Envia a sequência de comandos para os registradores internos via SPI
    MCP2515_setRegisters(txbuf->SIDH, data, 5 + frame->can_dlc);

    // Solicita a transmissão física (Request to Send)
    MCP2515_modifyRegister(txbuf->CTRL, TXB_TXREQ, TXB_TXREQ);

    // Aguarda um breve momento ou verifica se houve erro imediato de barramento
